  ...
  if (loading) return (
    <Layout>
      <div className="p-40 text-center animate-pulse space-y-8">
        ...
        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest italic">Loading profile...</p>
      </div>
    </Layout>
  );
      ...
      <div className="space-y-12 animate-in fade-in duration-700 pb-40">
        <PageHeader
          ...
          actions={
            <button
              ...
              disabled={saving}
              className="px-10 py-3.5 bg-indigo-600 text-white rounded-xl font-bold text-xs uppercase tracking-widest shadow-lg hover:bg-slate-950 transition-all active:scale-95 disabled:opacity-50"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 font-sans">
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="bg-slate-900 p-8 rounded-2xl text-white shadow-xl relative overflow-hidden group border border-slate-800">
              <div className="relative z-10">
                ...
                <p className="text-lg font-bold italic">Verified Professional</p>
              </div>
          ...
          <div className="lg:col-span-9">
            <div className="bg-white rounded-[2.5rem] p-8 lg:p-12 shadow-sm border border-slate-200 min-h-[800px] relative font-sans">
              <div className="relative z-10">
                {activeSection === 'basic' && (
                  <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="pt-20 grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-3">
                        ...
                        <input type="text" value={profile.name} onChange={e => setProfile({ ...profile, name: e.target.value })} className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-xl font-bold text-slate-900 outline-none focus:bg-white focus:border-indigo-400 transition-all italic" />
                      </div>
                      <div className="space-y-3">
                      ...
                      <div className="md:col-span-2 space-y-3">
                        ...
                        <textarea rows="5" value={profile.profile.summary} onChange={e => setProfile({ ...profile, profile: { ...profile.profile, summary: e.target.value } })} placeholder="Briefly describe your professional background and goals..." className="w-full px-8 py-6 bg-slate-50 border border-slate-200 rounded-[2rem] text-base font-semibold text-slate-700 outline-none focus:bg-white focus:border-indigo-400 transition-all resize-none italic leading-relaxed shadow-inner" />
                      </div>
                    </div>
                  </div>
                )}
                ...
                {activeSection === 'experience' && (
                  <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
                    <div className="flex justify-between items-center gap-4">
                      <h2 className="text-3xl font-bold text-slate-900 uppercase italic tracking-tight">Employment History</h2>
                      <button onClick={addExperience} className="px-6 py-2.5 bg-slate-900 text-white rounded-xl font-bold text-[10px] uppercase tracking-widest hover:bg-slate-800 transition-all shadow-md italic">+ Add Record</button>
                    </div>
                        ...
                        <div key={idx} className="group/item bg-white p-8 rounded-3xl border border-slate-200 shadow-sm hover:shadow-lg transition-all relative overflow-hidden">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                            <div className="space-y-2">
                              ...
                              <input type="text" value={exp.company} onChange={e => updateItem('experience', idx, 'company', e.target.value)} placeholder="Organization Name" className="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-xl text-lg font-bold text-slate-900 outline-none focus:bg-white focus:border-indigo-400 transition-all italic" />
                            </div>
                            <div className="space-y-2">
                              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Role / Position</label>
                              <input type="text" value={exp.position} onChange={e => updateItem('experience', idx, 'position', e.target.value)} placeholder="e.g. Lead Developer" className="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-xl text-lg font-bold text-slate-900 outline-none focus:bg-white focus:border-indigo-400 transition-all italic" />
                            </div>
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <textarea rows="4" value={exp.description} onChange={e => updateItem('experience', idx, 'description', e.target.value)} placeholder="Detail your contributions and key results..." className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-medium text-slate-600 outline-none focus:bg-white focus:border-indigo-400 transition-all resize-none italic leading-relaxed" />
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
                    <div className="bg-slate-900 p-10 rounded-3xl text-white shadow-xl relative overflow-hidden">
                      <div className="relative z-10 space-y-8">
                        <h2 className="text-3xl font-bold uppercase italic tracking-tight">Capacities & Skills</h2>
                        <div className="flex gap-4">
                          <input type="text" value={skillInput} onChange={e => setSkillInput(e.target.value)} onKeyPress={e => e.key === 'Enter' && addSkill()} placeholder="e.g. React.js, Strategic Planning..." className="flex-1 bg-white/10 border border-white/10 rounded-xl px-6 py-3.5 text-lg font-bold outline-none focus:bg-white/20 focus:border-indigo-400 transition-all italic" />
                          <button onClick={() => addSkill()} className="px-8 bg-indigo-600 text-white rounded-xl font-bold text-[11px] uppercase tracking-widest hover:bg-white hover:text-slate-900 transition-all shadow-lg italic">Add Skill</button>
                        </div>
                      </div>
                    ...
                    <div className="bg-slate-50 p-8 rounded-[2rem] border border-slate-200 shadow-inner flex flex-wrap gap-4 min-h-[300px] content-start">
                      {profile.profile.skills.map((s, idx) => (
                        <div key={idx} className="group/tag flex items-center gap-3 bg-white px-5 py-3 rounded-2xl shadow-sm border border-slate-300 hover:border-indigo-600 hover:-translate-y-1 transition-all italic">
                      ...
                      {profile.profile.skills.length === 0 && <div className="w-full flex flex-col items-center justify-center opacity-20 py-20 grayscale">
                        ...
                        <span className="text-lg font-bold uppercase tracking-widest italic">No skills added yet.</span>
                      </div>}
                    </div>
                  </div>
                )}
                ...
                {['education', 'projects'].includes(activeSection) && (
                  <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
                    <div className="flex justify-between items-center gap-4">
                      <h2 className="text-3xl font-bold text-slate-900 uppercase italic tracking-tight">{activeSection === 'education' ? 'Academic Background' : 'Key Projects'}</h2>
                      <button onClick={activeSection === 'education' ? addEducation : addProject} className="px-6 py-2.5 bg-slate-900 text-white rounded-xl font-bold text-[10px] uppercase tracking-widest hover:bg-slate-800 transition-all shadow-md italic">+ Add Record</button>
                    </div>
                    ...
                    <div className="space-y-8">
                      {profile.profile[activeSection].map((item, idx) => (
                        <div key={idx} className="group/item bg-white p-8 rounded-3xl border border-slate-200 shadow-sm hover:shadow-lg transition-all relative">
                          <button onClick={() => removeItem(activeSection, idx)} className="absolute top-4 right-4 w-10 h-10 bg-slate-50 text-slate-400 rounded-xl flex items-center justify-center text-xl hover:bg-rose-600 hover:text-white transition-all shadow-sm">✕</button>

                          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                            <div className="space-y-2">
                              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1"> {activeSection === 'education' ? 'Institution' : 'Project Name'} </label>
                              <input type="text" value={activeSection === 'education' ? item.institution : item.name} onChange={e => updateItem(activeSection, idx, activeSection === 'education' ? 'institution' : 'name', e.target.value)} placeholder={activeSection === 'education' ? 'University Name' : 'Enterprise Solution'} className="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-xl text-lg font-bold text-slate-900 outline-none focus:bg-white focus:border-indigo-400 transition-all italic" />
                            </div>
                            <div className="space-y-2">
                              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1"> {activeSection === 'education' ? 'Degree / Qualification' : 'Reference / URL'} </label>
                              <input type={activeSection === 'education' ? 'text' : 'url'} value={activeSection === 'education' ? item.degree : (item.url || '')} onChange={e => updateItem(activeSection, idx, activeSection === 'education' ? 'degree' : 'url', e.target.value)} placeholder={activeSection === 'education' ? 'e.g. BSc Computer Science' : 'https://github.com/...'} className="w-full px-5 py-3 bg-slate-50 border border-slate-100 rounded-xl text-lg font-bold text-slate-900 outline-none focus:bg-white focus:border-indigo-400 transition-all italic" />
                            </div>
                          </div>
                          <div className="space-y-2">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1"> Details & Description </label>
                            <textarea rows="4" value={item.description || item.field || ''} onChange={e => updateItem(activeSection, idx, activeSection === 'education' ? 'field' : 'description', e.target.value)} placeholder="Provide specific details about this record..." className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-medium text-slate-600 outline-none focus:bg-white focus:border-indigo-400 transition-all resize-none italic leading-relaxed" />
                          </div>
                        </div>
                      ))}
                    </div>
