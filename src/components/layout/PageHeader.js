        ...
        <div className={`${gradient} ${color} rounded-xl px-6 py-5 mb-8 relative overflow-hidden transition-all duration-300 font-sans`}>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                ...
                <div className="flex items-center gap-6 flex-wrap md:flex-nowrap">
                    {stats && stats.length > 0 && (
                        <div className="flex items-center gap-3">
                            {stats.map((stat, index) => (
                                <div
                                    key={index}
                                    className="bg-slate-50 border border-slate-100 px-3 py-1.5 rounded-lg min-w-[90px] transition-all hover:bg-white hover:shadow-sm hover:border-indigo-100"
                                >
                    ...
                    {actions && (
                        <div className="flex items-center gap-3 shrink-0 md:border-l md:border-slate-200 md:pl-6">
                            {actions}
                        </div>
                    )}
                </div>
