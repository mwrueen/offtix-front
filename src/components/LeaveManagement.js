...
const LeaveManagement = () => {
  ...
  const renderLeaveBalance = () => {
    ...
    return (
      <div className="bg-white p-8 lg:p-12 rounded-3xl border border-slate-200 shadow-sm mb-12 animate-in fade-in duration-500">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {Object.entries(leaveBalance).map(([type, data]) => (
            <div key={type} className="p-8 bg-slate-50 border border-slate-100 rounded-2xl transition-all hover:bg-white hover:shadow-xl hover:border-indigo-100 group">
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4">{getLeaveTypeLabel(type)}</div>
              <div className="flex items-baseline gap-2 mb-6">
                <span className="text-4xl font-bold tracking-tight text-slate-900 group-hover:text-indigo-600 transition-colors">
                  {data.remaining === Infinity ? '∞' : data.remaining}
                </span>
                <span className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">/ {data.total === Infinity ? 'Unlimited' : `${data.total} Days`}</span>
              </div>
  ...
  const renderLeaveCard = (leave, index) => {
    ...
    return (
      <div
        key={leave._id}
        className={`group bg-white p-6 lg:p-8 rounded-3xl border border-slate-200 shadow-sm hover:shadow-xl transition-all duration-300 relative overflow-hidden flex flex-col lg:flex-row animate-in slide-in-from-bottom-4`}
  ...
  if (loading) return (
    <Layout>
      <div className="flex flex-col items-center justify-center py-40 animate-pulse space-y-6">
        <div className="w-14 h-14 border-4 border-slate-100 border-t-indigo-600 rounded-full animate-spin shadow-sm" />
        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest italic">Synchronizing leave data...</p>
      </div>
    </Layout>
  );
  ...
  return (
    <Layout>
      <div className="space-y-12 animate-in fade-in duration-700 pb-40">
        ...
        <div className="flex flex-wrap gap-2 p-1.5 bg-white rounded-2xl border border-slate-200 w-fit shadow-md mb-8">
          {[
            ...
            <button
              ...
              className={`px-8 py-3.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all duration-300 relative ${activeTab === tab.id ? 'bg-slate-950 text-white shadow-lg' : 'text-slate-400 hover:text-slate-900 hover:bg-slate-50'}`}
            >
              {tab.label}
              {tab.id === 'pending-approvals' && leaves.filter(l => l.status === 'pending').length > 0 && activeTab !== tab.id && (
                <span className="absolute -top-1 -right-1 flex h-4 w-4">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-rose-400 opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-4 w-4 bg-rose-500 border-2 border-white items-center justify-center text-[8px] text-white leading-none font-black">{leaves.filter(l => l.status === 'pending').length}</span>
                </span>
              )}
            </button>
          ))}
        ...
        <div className="space-y-8 lg:space-y-10">
          {leaves.length === 0 ? (
            <div className="bg-white p-40 rounded-3xl text-center border-2 border-dashed border-slate-200 animate-in zoom-in-95 group">
              <div className="text-8xl mb-8 opacity-10 group-hover:scale-110 group-hover:opacity-20 transition-all duration-500">🗓️</div>
              <h3 className="text-2xl font-bold text-slate-300 uppercase tracking-widest">No Leave Records</h3>
              <p className="text-xs font-bold text-slate-400 uppercase mt-4 max-w-sm mx-auto opacity-60">There are no leave requests matching the current filter parameters.</p>
              <button onClick={() => setShowRequestModal(true)} className="mt-10 px-10 py-3.5 bg-indigo-600 text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-slate-950 transition-all shadow-lg active:scale-95">Create New Request</button>
            </div>
          ) : (
        ...
        {showRequestModal && (
          <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md flex items-center justify-center z-[2500] p-6 animate-in fade-in duration-300" onClick={() => setShowRequestModal(false)}>
            <div className="bg-white rounded-[2.5rem] w-full max-w-2xl shadow-2xl border border-white/20 overflow-hidden relative animate-in zoom-in-95 duration-500 max-h-[95vh] flex flex-col" onClick={e => e.stopPropagation()}>
              <div className="px-10 py-10 bg-slate-950 text-white flex justify-between items-center shrink-0">
                <div>
                  <h3 className="text-2xl font-bold tracking-tight">Time Off Request</h3>
                ...
                <button onClick={() => setShowRequestModal(false)} className="w-12 h-12 rounded-2xl bg-white/10 text-white hover:bg-rose-600 transition-all text-3xl font-bold flex items-center justify-center">×</button>
              </div>
